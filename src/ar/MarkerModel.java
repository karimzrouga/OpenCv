...
public class MarkerModel
{
  ...
  protected TransformGroup moveTg;      // for moving the marker model
  PropManager propMan;

  protected Switch visSwitch;           // for changing the model's visibility
  protected boolean isVisible;

  protected SmoothMatrix sMat;          // for smoothing the transforms applied to the model
 ...
 /* public static TransformGroup loadModelstatic(String modelFnm, double scale, boolean hasCoords)
  // load the model, rotating and scaling it
  {
    ...
    modelT3d.rotX( Math.PI/2.0 );    
         // the prop lies flat on the marker; rotate forwards 90 degrees so it is standing
    Vector3d scaleVec = calcScaleFactorstatic(propTG, scale);   // scale the prop
    modelT3d.setScale( scaleVec );
...
protected Vector3d calcScaleFactor(TransformGroup modelTG, double scale)
...
/////////////////
  protected static Vector3d calcScaleFactorstatic(TransformGroup modelTG, double scale)
  // Scale the prop based on its original bounding box size
  {
     BoundingBox boundbox = new BoundingBox( modelTG.getBounds() );
     System.out.println(boundbox);
  ...
  public void setMoveTg(TransformGroup nouveatTG)
  {  
	  moveTg=nouveatTG; 
  }
  ...
  {
    visSwitch.setWhichChild( Switch.CHILD_ALL );   // make models visible
    isVisible = true;
  ...
  protected void calcPosition(Matrix4d mat)
  ...
  protected void calcEulerRots(Matrix4d mat)
